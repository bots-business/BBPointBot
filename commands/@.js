...
function canMasterRemove(value) {
  ...
  if (pvalue > masterRemoveableLimit.value()) {
    return false
  }

  var masterRemoveablePoint = Libs.ResourcesLib.userRes("masterRemoveablePoint")
  if (masterRemoveablePoint.value() - pvalue < 0) {
    return false
  }
  return true
}
...
function broadcastOperation(amount, from_user, to_user, operation) {
  ...
  broadcastToChanell(
    operation + name + " > " + other_name + ": " + prefix + amount + "💎"
  )
}

function transferPoint() {
  res.removeAnyway(1)
  anotherRes = Libs.ResourcesLib.anotherUserRes(
    "BBPoint",
    answer.answer_from_id
  )
  anotherRes.add(1)
}
